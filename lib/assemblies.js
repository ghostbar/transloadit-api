...
module.exports = Assemblies = (function () {
  ...
  Assemblies.prototype.post = function (files, params, callback) {
    ...
    _.each(files, function(file) {
      console.log(file);
      var random = fs.createReadStream(file.path);
      random.on('data', function() {
        console.log("I DO HAVE EVENTS! :(");
      });
    ...
    form.submit(self.uri.base + self.uri.assemblies, function(err, response) {
      if (err)
        return callback(err);
